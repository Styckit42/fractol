...
int		expose_burning(mlx_t *mlx)
{
	...
	mlx->data_burning = mlx_get_data_addr(mlx->img_burning, &(mlx->bpp), &(mlx->sizeline), &(mlx->endian));
	reinit_burning(mlx);
	...
	mlx_put_image_to_window(mlx->mlx_ptr, mlx->win_burning, mlx->img_burning, 0, 0);
	return (0);
}
...
int		expose_mandel(mlx_t *mlx)
{
	...
	mlx->data_mandel = mlx_get_data_addr(mlx->img_mandel, &(mlx->bpp), &(mlx->sizeline), &(mlx->endian));
	reinit_mandel(mlx);
	...
	mlx_put_image_to_window(mlx->mlx_ptr, mlx->win_mandel, mlx->img_mandel, 0, 0);
	return (0);
}
