...
void	give_c_burning_value(mlx_t *mlx)
{
	...
	mlx->point_burning->ci = -(mlx->point_burning->y / mlx->limit_burning->coeffy
...
void		burning_while_calc(mlx_t *mlx)
{
	...
	while (i < mlx->limit_burning->iter && module < 4)
	{
		...
		mlx->point_burning->zr = mlx->point_burning->zr * mlx->point_burning->zr - mlx->point_burning->zi * mlx->point_burning->zi + mlx->point_burning->cr;
		mlx->point_burning->zi = 2 * ft_absolute_nb(mlx->point_burning->zi) * ft_absolute_nb(tmp) - mlx->point_burning->ci;
		module = mlx->point_burning->zr * mlx->point_burning->zr + mlx->point_burning->zi * mlx->point_burning->zi;
		i++;
	}
	if (i != mlx->limit_burning->iter)
		switch_color_burning(mlx, i);
	if (mlx->bonus_burning->bol_g == TRUE && (mlx->point_burning->x == WIDTH / 2 || mlx->point_burning->y == HEIGHT / 2))
		mlx_put_pixel_image(mlx->point_burning->x, mlx->point_burning->y, 0x00FF00 , *mlx, mlx->data_burning);
}
